...
start_timeout_cb (gpointer user_data)
{
	...
	handle_login_result (page, VPN_SERVICE_TIMEOUT_ERROR);
...
static void
show_error_dialog (GreeterVPNPage *page)
{
	const gchar *title;
	const gchar *message;
	GtkWidget   *dialog, *toplevel;
                         ...
                         gpointer   user_data)
{
	...
	if (!ip || g_str_equal (ip, "")) {
		handle_login_result (page, VPN_SERVICE_INFO_ERROR);
		goto out;
	}
	...
	if (!try_to_login (page, ip, port, id, pw))
